...
async def llm_chat(request: LlmChatRequest) -> LlmChatResponse:
    try:
        title, answer, model, chunk_count = await generate_llm_reply(request.data, request.question)
    except LlmInputTooLargeError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except HTTPError as exc:
        raise HTTPException(status_code=502, detail=f"LLM request failed: {exc}") from exc

    return LlmChatResponse(title=title, answer=answer, model=model, chunk_count=chunk_count)
